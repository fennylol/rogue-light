...
var spawn_spacing = 25
var wheel_radius: float = 1.625
...
static func init(hm: Array, ws: Vector3, ttc: float):
...
func get_spawn_spacing(): return spawn_spacing
...
func _process(delta): 
	...
	var heights = []
	for wheel in wheels:
		var h = height_at(wheel.get_global_position())+wheel_radius
		...
		wheel.get_child(0).rotation.z = -progress/wheel_radius
	...
	set_progress_ratio(get_progress_ratio()+delta/TIME_TO_COMPLETE)
